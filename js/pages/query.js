...
async function loadDatabasesForQuery() {
    try {
        ...
        // Sort databases to ensure consistent order
        const sortedDatabases = [...AppState.databases].sort((a, b) => 
            a.key === 'primary' ? -1 : b.key === 'primary' ? 1 : a.name.localeCompare(b.name)
        );
        ...
        // Ensure we have a selected database
        if (select.value) {
            AppState.selectedDatabase = select.value;
            AppState.currentDatabase = select.value;
            await updateQueryDatabase();
        } else if (sortedDatabases.length > 0) {
            // Default to first available database if none selected
            select.value = sortedDatabases[0].key;
            AppState.selectedDatabase = sortedDatabases[0].key;
            AppState.currentDatabase = sortedDatabases[0].key;
            await updateQueryDatabase();
        }
...
async function updateQueryDatabase() {
    ...
    const newDbKey = select.value;
    console.log('Updating selected database to:', newDbKey);
    
    // Update both current and selected database
    AppState.selectedDatabase = newDbKey;
    AppState.currentDatabase = newDbKey;
    ...
    if (newDbKey) {
        // Get the full database info from AppState
        const selectedDb = AppState.databases.find(db => db.key === newDbKey);
        if (!selectedDb) {
            console.error('Selected database not found in AppState:', newDbKey);
            showAlert('Error: Selected database not found', 'error');
            return;
        }
        ...
        showAlert(`Switched to database: ${dbName}`, 'info');
        ...
        try {
            ...
            const collections = await API.getCollections(newDbKey);
...
async function processQueryWithAPI(query) {
    try {
        ...
        const selectedDb = AppState.databases.find(db => db.key === dbKey);
        if (!selectedDb) {
            throw new Error(`Selected database ${dbKey} not found in available databases`);
        }
        
        console.log('Querying database:', dbKey, 'with top_k:', topK);
        console.log('Selected database info:', selectedDb);
        
        // Make API request with selected database
        const result = await API.post('/query/vector-rag', {
            query: query,
            db_key: dbKey,
            top_k: topK
        });
        ...
        // Process and format results data from sources
        ...
        // Store results and chart data in AppState
        ...
        // Show success message with answer if available
        if (result.answer) {
