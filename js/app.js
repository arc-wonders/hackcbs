...
function navigateTo(page) {
    // Check if we have data when navigating to results
    if (page === 'results' && (!AppState.queryResults || AppState.queryResults.length === 0)) {
    ...
    // If navigating to results, ensure chart initialization
    if (page === 'results') {
        setTimeout(() => {
            const resultsPage = document.getElementById('app-content');
            if (resultsPage && typeof initializeResultsPage === 'function') {
